...
public class NavigationListFragment extends Fragment implements BeaconHolder.IBeaconListener {
                        ...
                        ScheduleUtil.scheduleWork(new Runnable() {
                            ...
                            public void run() {
                                getActivity().runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        bt_to.sendAccessibilityEvent(AccessibilityEvent.TYPE_VIEW_FOCUSED);
                                    }
                                });
                            }
                        ...
                        ScheduleUtil.scheduleWork(new Runnable() {
                            ...
                            public void run() {
                                getActivity().runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        bt_calc.sendAccessibilityEvent(AccessibilityEvent.TYPE_VIEW_FOCUSED);
                                    }
                                });
                            }
                        }, 500, TimeUnit.MILLISECONDS);
        ...
        bt_calc.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (start != null && target != null) {
                    ...
                    new CalculateShortestPath().execute();
                } else {
    ...
    public void onBeaconChanged(BeaconLocation changedBeacon) {
        for (BeaconLocation b : resultList) {
            if (b.macAddress.equals(changedBeacon.macAddress)) {
                BeaconsViewHolder viewHolder = (BeaconsViewHolder) mRecyclerView.findViewHolderForAdapterPosition(
                ...
                if (changedBeacon.preRSSI < RangeThreshold.NEAR && changedBeacon.RSSI >= RangeThreshold.NEAR) {
                    ...
                    Toast.makeText(getActivity(), changedBeacon.placeId + this.getString(R.string.txt_is_very_close), Toast.LENGTH_SHORT)
                         .show();
                    ...
                    LocalBroadcastManager.getInstance(getActivity()).sendBroadcast(new Intent(getString(R.string.intent_gatt_open))
                                                                                           .putExtra(getString(R.string.bndl_mac),
                                                                                                     changedBeacon.macAddress));
                } else if (changedBeacon.preRSSI < RangeThreshold.FAR && changedBeacon.RSSI >= RangeThreshold.FAR) {
                    changedBeacon.preRSSI = changedBeacon.RSSI;
                    Toast.makeText(getActivity(), changedBeacon.placeId + this.getString(R.string.txt_is_in_range), Toast.LENGTH_SHORT)
                         .show();
                }
